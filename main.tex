...
\begin{frame}{Visualizing the Influence Function (1D)}
    ...
    \begin{tikzpicture}[scale=0.9, >=stealth]
        ...
        \draw[thick, blue!70!black, domain=0.5:7.8, smooth, samples=80]
            plot (\x, {0.18*(\x-4)^2 + 0.8})
            node[right, blue!70!black] {};
        \node[blue!70!black, above right] at (7, 3.5) {$\frac{1}{n}\sum_i L(z_i, \theta)$};

        % Upweighted risk: R(theta) + epsilon * L(z, theta), shifted minimum to ~3.0
        \draw[thick, red!70!black, domain=0.5:7.8, smooth, samples=80]
            plot (\x, {0.22*(\x-3.0)^2 + 1.1})
            node[right, red!70!black] {};
        \node[red!70!black, above left] at (1.2, 4.0) {$\frac{1}{n}\sum_i L(z_i, \theta) + \epsilon L(z, \theta)$};

        % Mark hat{theta} (original minimum)
        \fill[blue!70!black] (4, 0.8) circle (3pt);
        \draw[dashed, blue!70!black] (4, 0) -- (4, 0.8);
        \node[below, blue!70!black] at (4, -0.05) {$\hat{\theta}$};

        % Mark hat{theta}_{epsilon,z} (new minimum)
        \fill[red!70!black] (3.0, 1.1) circle (3pt);
        \draw[dashed, red!70!black] (3.0, 0) -- (3.0, 1.1);
        \node[below, red!70!black] at (3.0, -0.05) {$\hat{\theta}_{\epsilon,z}$};

        % Influence arrow between the two minima
        \draw[<->, thick, black!60!green, line width=1.2pt] (3.05, 0.45) -- (3.95, 0.45);
        \node[above, black!60!green] at (3.5, 0.45) {$\epsilon \cdot \mathcal{I}_{\text{up}}(z)$};

        % Tangent line at hat{theta} on the upweighted loss (showing linear approx)
        \draw[dashed, orange, thick, domain=2.0:5.5, samples=2]
            plot (\x, {0.44*(4-3.0)*(\x - 4) + 0.22*(4-3.0)^2 + 1.1});
        \node[orange, right] at (5.5, 2.3) {\small linear approx.};
    \end{tikzpicture}
    \end{center}

    \vspace{-0.2cm}
    \begin{itemize}
        \item \textcolor{blue!70!black}{Blue}: original empirical risk, minimized at $\hat{\theta}$
        \item \textcolor{red!70!black}{Red}: upweighted loss (adding $\epsilon \, L(z, \theta)$), minimized at $\hat{\theta}_{\epsilon,z}$
        \item The \textcolor{black!60!green}{shift} is approximated by the influence function without retraining
    \end{itemize}
\end{frame}
